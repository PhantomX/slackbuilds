...
if [ "${SB_PA}" = "YES" ] ;then
  zcat ${SB_PATCHDIR}/${PNAME}-1.2.13-audiodriver.patch.gz | patch -p1 -E --backup --verbose || exit 1
fi

# submitted upstream:
# http://bugzilla.libsdl.org/show_bug.cgi?id=698
zcat ${SB_PATCHDIR}/${PNAME}-1.2.13-pa-rewrite.patch.gz | patch -p1 -E --backup --verbose || exit 1
# submitted upstream:
# http://bugzilla.libsdl.org/show_bug.cgi?id=699
zcat ${SB_PATCHDIR}/${PNAME}-1.2.13-rh484362.patch.gz | patch -p1 -E --backup --verbose || exit 1
# upstream patch:
# qemu-kvm segfaults on startup in SDL_memcpyMMX/SSE
zcat ${SB_PATCHDIR}/${PNAME}-1.2.13-rh487720.patch.gz | patch -p1 -E --backup --verbose || exit 1
