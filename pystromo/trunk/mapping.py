...
class Mapper (_BaseConfig):
	...
	def __init__ (self, *args):
		_BaseConfig.__init__ (self, *args)
	...
	def __contains__ (self, key):
		...
		if isinstance(key, Key):
			...
			for device, maps in self.mappings.iteritems():
	...
	def deviceContains (self, key, device=None):
		"""
	...
	def remap (self, chord, device=None):
		"""
		...
		if device in mappings:
			remapping = self._findChord(mappings[device], chord)
		else:
			remapping = self._findChord(mappings[None], chord)
		...
		if None not in (inMin, inMax, chord.min) and chord.min == chord.max:
			# Provide an output tailored to the value of the chord
			if inMin == inMax:
	...
	def _findChord (self, mapDict, chord):
		"""
