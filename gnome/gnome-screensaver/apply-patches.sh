...
zcat ${SB_PATCHDIR}/${NAME}-2.18.0-securitytoken.patch.gz | patch -p1 -E --backup --verbose || exit 1
# add support for gnome-keyring to the pam stack
#zcat ${SB_PATCHDIR}/gnome-keyring.patch.gz | patch -p1 -E --backup --verbose || exit 1
zcat ${SB_PATCHDIR}/${NAME}-2.20.0-restore-gamma.patch.gz | patch -p1 -E --backup --verbose || exit 1
