...
patch -p1 -E --backup --verbose -i ${SB_PATCHDIR}/python-3.2rc1-no-static-lib.patch
if [ "${ARCH}" = "x86_64" ]; then
  patch -p1 -E --backup --verbose -i ${SB_PATCHDIR}/python-3.2b2-lib64.patch
fi
...
patch -p1 -E --backup --verbose -i ${SB_PATCHDIR}/python-3.2b2-fix-test-subprocess-with-nonreadable-path-dir.patch
