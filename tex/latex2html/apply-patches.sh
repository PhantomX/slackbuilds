...
zcat ${SB_PATCHDIR}/teTeX-l2h-config.patch.gz | patch -p2 -E --backup --verbos
# Fix latex2html not to use DB_File
zcat ${SB_PATCHDIR}/${NAME}-2K.1beta-DB.patch.gz | patch -p2 -E --backup --verbos
# fix SHLIBDIR
zcat ${SB_PATCHDIR}/${NAME}-2002-2-1-SHLIB.patch.gz | patch -p1 -E --backup --verbos
# don't require the font directory to be ended with PATH/fonts
zcat ${SB_PATCHDIR}/${NAME}-2002-2-1-gsfont.patch.gz | patch -p1 -E --backup --verbos
