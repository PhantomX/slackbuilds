...
zcat ${SB_PATCHDIR}/${NAME}-0.11.0-getaddrinfo.patch.gz | patch -p1 -E --backup --verbose
...
zcat ${SB_PATCHDIR}/${NAME}-0.10.1-xterm.patch.gz | patch -p1 -E --backup --verbose
# Fix #157300 - Strange behavior on ppc64
zcat ${SB_PATCHDIR}/${NAME}-0.11.0-union.patch.gz | patch -p1 -E --backup --verbose
# fix for open macro in new glibc 
zcat ${SB_PATCHDIR}/${NAME}-0.11.3-macropen.patch.gz | patch -p1 -E --backup --verbose
#upstream fix for out of screen dialogs
zcat ${SB_PATCHDIR}/${NAME}-scroll.patch.gz | patch -p1 -E --backup --verbose
# Port elinks to use NSS library for cryptography
zcat ${SB_PATCHDIR}/${NAME}-nss.patch.gz | patch -p1 -E --backup --verbose
